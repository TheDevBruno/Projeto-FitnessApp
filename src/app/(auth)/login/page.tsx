...
export default async function LoginPage({
  ...
  const signIn = async (formData: FormData) => {
    ...
    if (error) return redirect('/login?message=Email ou senha inválidos');
  ...
  const signInWithGoogle = async () => {
    ...
    if (error) return redirect('/login?message=Erro ao autenticar com Google');
    ...
    <div className={styles.container}>
      <div className={styles.card}>
        ...
        <form className={styles.form}>
          ...
          <button formAction={signIn} className="btn btn-primary" style={{ width: '100%' }}>
            Entrar
          </button>
          ...
          <button formAction={signInWithGoogle} formNoValidate className={`btn btn-secondary ${styles.googleBtn}`}>
